...
如何保证一个对象只能被实例化一次？利用静态变量、静态方法和适当的访问修饰符，可以实现。而更好的方法是让单例类自身负责管理它的唯一实例。
...
```
私有静态变量 instance 被延迟实例化，他好处是如果没有用到该类，那么就不会实例化，从而节约资源。但这个实现在多线程环境下是不安全的，如果多个线程能够同时进入 if (instance == null) ，并且此时 instance 为 null，那么会有多个线程执行 instance = new Singleton();
